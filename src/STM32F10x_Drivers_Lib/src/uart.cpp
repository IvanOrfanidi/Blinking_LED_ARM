...
Uart::~Uart()
{
	...
	// Disable USARTs clock
	disableUartClock();

	// Disable pins Hardware Flow Control
...
void Uart::createInterrupt(uint8_t preemption, uint8_t sub)
{
	...
	// Enable the USARTx receive interrupt: this interrupt is generated when
	// uart receive data register is not empty
	enableReceive();
}
...
void Uart::handler()
{
	...
	if(isSetInterruptFlag(USART_IT_TXE)) {
		if(!_transmitBuffer->empty()) {
			const auto data = _transmitBuffer->pop();
	...
	if(isSetInterruptFlag(USART_IT_RXNE)) {
		...
		if(!isError) {
			auto data = static_cast<uint8_t>(read());
